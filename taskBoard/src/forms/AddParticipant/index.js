...
const AddParticipantForm = ({ projectId, taskId, onSubmitPress, onCancelPress }) => {
  ...
  const handleDeleteSelectedUser = user => {
    setPossibleParticipants(possibleParticipants => [...possibleParticipants, user])
    setSelectedUsers(selectedUsers => selectedUsers.filter(selectedUser => selectedUser.id !== user.id))
  }
  ...
  return loading ? (
    ...
    <Form>
      ...
      <FormButton onClick={() => onSubmitPress(selectedUsers.map(selectedUser => selectedUser.id))}>
        Add users
      </FormButton>
